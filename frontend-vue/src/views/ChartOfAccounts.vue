    ...
    <div class="card">
      <div class="table-container">
        <table>
          ...
          <tbody>
            <tr v-for="item in coaData" :key="item.code">
              <td>
                <span style="font-family: monospace; font-weight: 700; color: #2563eb; font-size: 13px;">
                  {{ item.code }}
                </span>
              </td>
              <td><strong>{{ item.name }}</strong></td>
              <td>
                <span :class="['badge', item.type === 'Aset' ? 'badge-blue' : item.type === 'Pendapatan' ? 'badge-green' : item.type === 'Beban Amil' ? 'badge-orange' : 'badge-purple']">
                  {{ item.type }}
                </span>
              </td>
              <td>{{ item.fund }}</td>
              <td style="font-size: 12px; color: #475569;">{{ item.desc }}</td>
            </tr>
          </tbody>
        </table>
      </div>
